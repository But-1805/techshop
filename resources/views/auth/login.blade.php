            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="mb-4">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg" autocomplete="email" autofocus>
                    @error('email')
                ...
                <div class="mb-6">
                    ...
                    <input id="password" type="password" name="password" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg" autocomplete="current-password">
                    @error('password')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
                <button type="submit" class="w-full bg-black text-white font-bold py-3 rounded-lg">Đăng Nhập</button>
    ...
    @if (session('success'))
        <script>
            Toastify({
                ...
                duration: 5000, // Để thời gian lâu hơn cho người dùng đọc
                gravity: "top",
                position: "right",
                style: {
